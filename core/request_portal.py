...
def review_company_request(request_id: str, reviewer_username: str, approved: bool, review_note: str | None = None) -> None:
    ensure_request_tables()
    with engine.begin() as connection:
        request_row = connection.execute(
            text(
                """
                SELECT request_id, company_name, source_url, review_status
                FROM tech.ls_company_requests
        ...
        if approved:
            _apply_company_request(connection, str(request_row["company_name"]), str(request_row["source_url"]))
...
def _apply_company_request(connection, company_name: str, source_url: str) -> None:
